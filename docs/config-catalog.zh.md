...
export interface Config {
  /** Xingyunxunzhi home directory exposed as `DSH_HOME`; defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
}
```
...
export interface Config {
  ...
  /** Xingyunxunzhi config root. Defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
...
export interface Config {
  /** Include the fixed Xingyunxunzhi identity before the deployment persona (default true). */
