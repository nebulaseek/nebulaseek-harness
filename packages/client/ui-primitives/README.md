...
| `icons/*`, `BrandLogo`, `BrandWordmark`, `ReferenceIcon`, `LinkIcon`, `DocumentFileIcon` | Glyphs and brand marks; the glyphs ride `currentColor`, the brand artwork carries its own gradients. |
...
- **Glyph-level icons are redrawn approximations** — the sparkle mark comes from a font glyph whose vector geometry is not exportable from the local design data; a hand-authored recreation stands in until an exact export path exists. `BrandLogo` and `BrandWordmark` are exempt: their artwork comes from the brand source file.
