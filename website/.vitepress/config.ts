...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/xingyunxunzhi/xingyunxunzhi-harness' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/xingyunxunzhi/xingyunxunzhi-harness/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
const siteIdentity = {
  title: '星云寻知',
  description: '用于构建 Agent 内核的插件化 SDK',
}

/**
 * The brand wordmark, inlined so its `currentColor` fills follow the active
...
/**
 * Navigation-bar title: the brand wordmark and the release-stage tag.
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'xingyunxunzhi-doc-projector',
