...
class HarnessConfig:
    """Configuration for launching the local Xingyunxunzhi SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Xingyunxunzhi SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Xingyunxunzhi runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Xingyunxunzhi runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Xingyunxunzhi runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Xingyunxunzhi runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Xingyunxunzhi runtime stdout closed"))
        ...
        if self.config.dsh_bin is None:
            try:
                ...
                raise FileNotFoundError(
                    "Unable to locate the bundled Xingyunxunzhi dsh runtime. "
